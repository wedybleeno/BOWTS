...
def download_audio_from_url(url, category="music", callback=None):
    ...
    ydl_opts = {
        ...
        'writethumbnail': True,  # Сохраняем превью
    }
    ...
    # Если есть превью, сохраняем его с тем же именем
    final_thumb_path = None
    if thumbnail_path:
        thumb_ext = os.path.splitext(thumbnail_path)[1]
        final_thumb_path = os.path.join(AUDIO_DIR, clean + thumb_ext)
        os.replace(thumbnail_path, final_thumb_path)
    ...
    categories[final_fn] = {
        ...
        "thumbnail": os.path.basename(final_thumb_path) if final_thumb_path else None
    }
    save_meta()
...
class AudioBotApp(QMainWindow):
    ...
    def create_ui(self):
        ...
        # Виджет превью для источника
        self.preview_container = QWidget()
        self.preview_container.setMinimumHeight(300)
        preview_layout = QVBoxLayout(self.preview_container)
        
        # Заголовок превью
        preview_header = QLabel("Источник контента")
        preview_header.setAlignment(Qt.AlignCenter)
        preview_header.setStyleSheet("font-weight: bold; color: #3498db;")
        
        # Виджет для отображения превью
        self.preview_label = QLabel()
        self.preview_label.setAlignment(Qt.AlignCenter)
        self.preview_label.setMinimumSize(480, 270)  # Соотношение 16:9
        ...
        player_layout.addWidget(self.preview_container)
    ...
    def update_preview(self, metadata):
        ...
        if thumbnail:
            # Загружаем изображение и устанавливаем в превью
            thumb_path = os.path.join(AUDIO_DIR, thumbnail)
            if os.path.exists(thumb_path):
                pixmap = QPixmap(thumb_path)
                if not pixmap.isNull():
                    self.preview_label.setPixmap(pixmap)
        else:
            # Если нет превью, показываем заглушку
            self.preview_label.setText("Нет доступного превью")
        
        # Показываем ссылку на источник если она есть
        if source_url:
            self.source_label.setText(f'<a href="{source_url}" style="color: #3498db;">Открыть источник</a>')
        else:
            self.source_label.setText("Источник недоступен")
